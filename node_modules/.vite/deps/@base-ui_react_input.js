import {
  useControlled
} from "./chunk-YHG6UG3X.js";
import {
  error
} from "./chunk-HQPNHHNY.js";
import {
  useRegisteredLabelId
} from "./chunk-ZN5JUZUY.js";
import {
  useTimeout
...
var FieldRootInner = React7.forwardRef(function FieldRootInner2(componentProps, forwardedRef) {
  const {
    errors,
  ...
  const [touchedState, setTouchedUnwrapped] = React7.useState(false);
  const [dirtyState, setDirtyUnwrapped] = React7.useState(false);
  const [filled, setFilled] = React7.useState(false);
  const [focused, setFocused] = React7.useState(false);
  const dirty = dirtyProp ?? dirtyState;
  const touched = touchedProp ?? touchedState;
  const markedDirtyRef = React7.useRef(false);
  ...
  const [validityData, setValidityData] = React7.useState({
    state: DEFAULT_VALIDITY_STATE,
    error: "",
    errors: [],
    value: null,
  ...
  const handleImperativeValidate = React7.useCallback(() => {
    markedDirtyRef.current = true;
    validation.commit(validityData.value);
  }, [validation, validityData]);
  React7.useImperativeHandle(actionsRef, () => ({
...
var FieldRoot = React7.forwardRef(function FieldRoot2(componentProps, forwardedRef) {
...
function useLabelableId(params = {}) {
  ...
  const hasRegisteredRef = React11.useRef(false);
  const hadExplicitIdRef = React11.useRef(id != null);
...
// node_modules/@base-ui/react/esm/field/control/FieldControl.js
var FieldControl = React12.forwardRef(function FieldControl2(componentProps, forwardedRef) {
  const {
    render,
    className,
    id: idProp,
...
var FieldValidity = function FieldValidity2(props) {
  ...
  const combinedFieldValidityData = React13.useMemo(() => getCombinedFieldValidityData(validityData, invalid), [validityData, invalid]);
  const isInvalid = combinedFieldValidityData.state.valid === false;
  const {
    transitionStatus
  } = useTransitionStatus(isInvalid);
  const fieldValidityState = React13.useMemo(() => {
    return {
...
var FieldItemContext = React14.createContext({
  disabled: false
});
...
var FieldItem = React16.forwardRef(function FieldItem2(componentProps, forwardedRef) {
  const {
    render,
    className,
    disabled: disabledProp = false,
  ...
  const fieldItemContext = React16.useMemo(() => ({
    disabled
  }), [disabled]);
  const element = useRenderElement("div", componentProps, {
    ref: forwardedRef,
