...
async function initDB() {
  try {
    ...
    const ambCount = await pool.query('SELECT COUNT(*) as count FROM ambulance');
    if (parseInt(ambCount.rows[0].count) === 0) {
      // Seed staff
      await pool.query(`INSERT INTO staff (staff_id, first_name, last_name, role, email, phone_number) VALUES 
      ...
      // Seed hospital
      await pool.query(`INSERT INTO hospital (hospital_id, hospital_name, contact_number, latitude, longitude, total_er_beds, available_er_beds, available_icu_beds, available_maternity_beds, specialties) VALUES 
      ...
      // Seed ambulance
      await pool.query(`INSERT INTO ambulance (ambulance_id, vehicle_number, ambulance_type, status, driver_id, paramedic_id, base_hospital_id, current_latitude, current_longitude, last_maintenance_date) VALUES 
      ...
      // Seed patient
      await pool.query(`INSERT INTO patient (patient_id, full_name, phone_number, clinical_notes) VALUES 
        ('44444444-4444-4444-4444-444444444444', 'Kamal Hossain', '01711223344', '{"allergies": ["Penicillin"], "conditions": ["Asthma"]}')
        ON CONFLICT DO NOTHING;`);

      // Seed emergency_request
      await pool.query(`INSERT INTO emergency_request (request_id, patient_id, ambulance_id, destination_hospital_id, status, pickup_latitude, pickup_longitude) VALUES 
      ...
      // Seed clusters
      await pool.query(`INSERT INTO clusters (id, name, lat, lng, radius, "ambulanceIds", "coverageScore") VALUES 
        ('CL-1', 'Gulshan-Banani', 23.7940, 90.4125, 3.0, '["33333333-3333-3333-3333-333333333331"]', 90)
        ON CONFLICT DO NOTHING;`);
    }
  } catch (err) {
    console.warn("Database connection failed. Running in memory-mock mode for preview.");
    dbConnected = false;
  }
}
  ...
  app.get('/api/clusters', async (req, res) => {
    ...
    try {
      ...
      const clusters = result.rows.map((r: any) => ({
        ...
        radius: r.radius,
        ambulanceIds: JSON.parse(r.ambulanceIds),
